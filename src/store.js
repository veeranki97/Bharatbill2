...
export const saveWorkOrder = async (wo, { overwrite = false } = {}) => {
  const qs = overwrite ? '?overwrite=1' : '';
  return apiFetch(`${API}/workorders${qs}`, { method: 'POST', body: JSON.stringify(wo) });
};

export const deleteWorkOrder = async (id) => {
  return apiFetch(`${API}/workorders/${encodeURIComponent(id)}`, { method: 'DELETE' });
};
...
export const savePurchaseOrder = async (po, { overwrite = false } = {}) => {
  const qs = overwrite ? '?overwrite=1' : '';
  return apiFetch(`${API}/purchaseorders${qs}`, { method: 'POST', body: JSON.stringify(po) });
};
export const deletePurchaseOrder = async (id) =>
  apiFetch(`${API}/purchaseorders/${encodeURIComponent(id)}`, { method: 'DELETE' });


// ---- Journals (simple double-entry — custom) ----
export const getAllJournals = async () => apiFetch(`${API}/journals`);
export const saveJournal = async (j) =>
  apiFetch(`${API}/journals`, { method: 'POST', body: JSON.stringify(j) });
...
export const getAllCostCenters = async () => apiFetch(`${API}/costcenters`);
export const saveCostCenter = async (cc) =>
  apiFetch(`${API}/costcenters`, { method: 'POST', body: JSON.stringify(cc) });
export const deleteCostCenter = async (id) =>
  apiFetch(`${API}/costcenters/${encodeURIComponent(id)}`, { method: 'DELETE' });
