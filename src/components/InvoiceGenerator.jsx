...
export default function InvoiceGenerator({ onBack, profile: profileProp, editingBill }) {
  ...
  useEffect(() => {
    ...
    // Load Work Orders for the link dropdown (custom)
  ...
  const saveInvoiceToDB = async (skipStockDeduction = false, extraPatch = {}) => {
    ...
    // === P0 validations (B2C GSTIN optional; PoS required; period range; e-way goods only) ===
    if (!(client.state || '').trim()) {
      toast('Place of Supply / Client State is mandatory (decides CGST+SGST vs IGST)', 'error');
      return;
    }

    // Credit limit (warn)
    try {
      const master = (savedClients || []).find(c => (c.name || '').toLowerCase() === (client.name || '').toLowerCase());
      const lim = Number(master?.creditLimit) || 0;
      if (lim > 0) {
        const open = (allBillsForCredit || []).filter(b =>
          (b.clientName || '').toLowerCase() === (client.name || '').toLowerCase()
          && b.status !== 'cancelled' && b.status !== 'paid'
        ).reduce((s, b) => s + Math.max(0, (Number(b.totalAmount)||0) - (Number(b.paidAmount)||0)), 0);
        const next = open + (Number(totals?.total) || 0);
        if (next > lim + 0.01) {
          const ok = confirm(`Credit limit ₹${lim.toLocaleString('en-IN')} exceeded (open+this ≈ ₹${next.toLocaleString('en-IN')}). Continue?`);
          if (!ok) return;
        }
      }
    } catch {}
    const gstinVal = (client.gstin || '').trim().toUpperCase();
    if (gstinVal) {
      if (gstinVal.length !== 15) {
        toast('GSTIN must be exactly 15 characters when provided', 'error');
        return;
      }
      try {
        const v = _isValidGSTIN(gstinVal);
        if (v && v.valid === false) {
          toast(v.message || 'Invalid GSTIN', 'error');
          return;
        }
      } catch { /* format length already checked */ }
    }
    const invDate = details.invoiceDate ? new Date(details.invoiceDate) : null;
    ...
    let ewayThreshold = 50000;
    try {
      const raw = localStorage.getItem('freegstbill_eway_threshold');
      if (raw != null && raw !== '') ewayThreshold = Number(raw) || 50000;
    } catch {}
    const hasGoodsLine = (items || []).some(it => {
      const h = String(it.hsn || '').trim();
      if (!h) return false;
      // SAC typically 6 digits starting with 99; treat pure numeric 4-8 digit non-99 as goods HSN
      if (/^99\d{4}$/.test(h)) return false;
      return /^\d{4,8}$/.test(h);
    });
    const grand = Number(totals?.total) || Number(bill?.totalAmount) || 0;
    if (hasGoodsLine && grand > ewayThreshold && !(details.eWayBillNo || '').trim()) {
      toast(`E-Way Bill number required for goods invoices above ₹${ewayThreshold.toLocaleString('en-IN')}`, 'error');
      return;
    }
    // Credit note must link original invoice
    const invType = (invoiceType || '').toLowerCase();
    if ((invType.includes('credit') || invType === 'credit-note') && !(details.originalInvoiceNumber || details.againstInvoice || '').trim()) {
      toast('Credit Note must link to the original invoice number', 'error');
      return;
    }


    // ===== WORK ORDER BUDGET CEILING CHECK (custom) =====
    if (bill.workOrderId) {
      try {
        ...
        console.error('WO ceiling check failed:', woErr);
        toast('Could not verify Work Order budget', 'error');
        return;
      }
    }
    // ===== END CEILING CHECK =====
    ...
    try {
      ...
      // Simple double-entry journal (custom) — tax invoices only; non-blocking
      try {
              ...
              <div className="invoice-options">
                ...
                {[
                  ...
                  { group: 'Items table', items: [
                    ...
                    // discount hidden for service ERP
