...
export default function GSTReturns() {
  ...
  // ===== GSTR-2A CSV reconciliation (custom, adapted from SD Dynamics) =====
  const [gstr2aRows, setGstr2aRows] = useState([]);
  const [gstr2aFilter, setGstr2aFilter] = useState('all');
  const gstr2aInputRef = useRef(null);

  const cleanInv = (inv) => String(inv || '').replace(/[^a-zA-Z0-9]/g, '').toUpperCase().trim();

  const parseGSTR2ACSV = (text) => {
    ...
    const headers = splitCsv(lines[0]).map(h => h.replace(/"/g, '').toLowerCase());
    const findCol = (...names) => {
    ...
    const iGstin = findCol('gstin', 'ctin');
    const iName = findCol('trade name', 'legal name', 'supplier', 'name');
    const iInv = findCol('invoice number', 'invoice no', 'inum', 'inv');
    const iDate = findCol('invoice date', 'idt', 'date');
    const iTaxable = findCol('taxable', 'txval', 'taxable value');
    const iIgst = findCol('igst');
    const iCgst = findCol('cgst');
    const iSgst = findCol('sgst');
    const iTax = findCol('total tax', 'tax amount', 'itc');
    const rows = [];
    for (let i = 1; i < lines.length; i++) {
      const cols = splitCsv(lines[i]);
      const gstin = (iGstin >= 0 ? cols[iGstin] : '').toUpperCase().trim();
      const invNo = iInv >= 0 ? cols[iInv] : '';
      if (!gstin && !invNo) continue;
      const igst = parseFloat(String(iIgst >= 0 ? cols[iIgst] : 0).replace(/,/g, '')) || 0;
      const cgst = parseFloat(String(iCgst >= 0 ? cols[iCgst] : 0).replace(/,/g, '')) || 0;
      const sgst = parseFloat(String(iSgst >= 0 ? cols[iSgst] : 0).replace(/,/g, '')) || 0;
      let tax = parseFloat(String(iTax >= 0 ? cols[iTax] : 0).replace(/,/g, '')) || 0;
      if (!tax) tax = igst + cgst + sgst;
      rows.push({
        gstin,
        name: iName >= 0 ? cols[iName] : '',
        invNo,
        date: iDate >= 0 ? cols[iDate] : '',
        taxable: parseFloat(String(iTaxable >= 0 ? cols[iTaxable] : 0).replace(/,/g, '')) || 0,
        tax,
      });
    }
    return rows;
  ...
  const build2AReconciliation = (portalRows, bookPurchases) => {
    const bookMap = {};
    (bookPurchases || []).forEach(p => {
      const key = (p.vendorGstin || p.gstin || '').toUpperCase().trim() + '_' + cleanInv(p.invoiceNumber || p.invNo || p.billNo);
      if (!key.endsWith('_')) bookMap[key] = p;
    });
    const matchedKeys = new Set();
    const results = [];
    (portalRows || []).forEach(r => {
      const key = (r.gstin || '').toUpperCase() + '_' + cleanInv(r.invNo);
      const book = bookMap[key];
      if (book) {
        matchedKeys.add(key);
        const bookTax = Number(book.taxAmount || book.tax || book.gstAmount || 0);
        const diff = Math.abs(bookTax - (r.tax || 0));
        results.push({
          ...r,
          bookTax,
          bookTotal: Number(book.totalAmount || book.total || 0),
          status: diff <= 2 ? 'matched' : 'amount_mismatch',
        });
      } else {
        results.push({ ...r, bookTax: 0, bookTotal: 0, status: 'portal_only' });
      }
    });
    (bookPurchases || []).forEach(p => {
      const key = (p.vendorGstin || p.gstin || '').toUpperCase().trim() + '_' + cleanInv(p.invoiceNumber || p.invNo || p.billNo);
      if (!key.endsWith('_') && !matchedKeys.has(key)) {
        results.push({
          gstin: p.vendorGstin || p.gstin || '',
          name: p.vendorName || p.vendor || '',
          invNo: p.invoiceNumber || p.invNo || p.billNo || '',
          date: p.date || p.invoiceDate || '',
          taxable: Number(p.taxable || 0),
          tax: Number(p.taxAmount || p.tax || p.gstAmount || 0),
          bookTax: Number(p.taxAmount || p.tax || p.gstAmount || 0),
          bookTotal: Number(p.totalAmount || p.total || 0),
          status: 'book_only',
        });
      }
    });
  ...
  const handleImport2A = async (e) => {
    ...
    try {
      ...
      if (!rows.length) {
        toast('No invoice rows found in CSV. Check portal GSTR-2A export format.', 'error');
        return;
      }
      setGstr2aRows(rows);
      toast(`Imported ${rows.length} GSTR-2A row(s)`, 'success');
    } catch (err) {
      console.error(err);
      toast('Failed to parse GSTR-2A CSV', 'error');
    }
    if (gstr2aInputRef.current) gstr2aInputRef.current.value = '';
  };
    ...
    <div className="dashboard-container">
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
        {[
          ...
          { id: 'gstr2b', label: 'GSTR-2B (ITC primary)', icon: CheckCircle },
          { id: 'gstr2a', label: 'GSTR-2A (secondary)', icon: CheckCircle },
          { id: 'gstliability', label: 'GST Liability Tracker', icon: BarChart3 },
      ...
      {/* ===================== GSTR-2A CSV RECONCILIATION (custom) ===================== */}
      {activeTab === 'gstr2a' && (() => {
        const recon = build2AReconciliation(gstr2aRows, purchases || []);
        const visible = gstr2aFilter === 'all' ? recon : recon.filter(r => r.status === gstr2aFilter);
        const counts = {
          matched: recon.filter(r => r.status === 'matched').length,
          amount_mismatch: recon.filter(r => r.status === 'amount_mismatch').length,
          portal_only: recon.filter(r => r.status === 'portal_only').length,
          book_only: recon.filter(r => r.status === 'book_only').length,
        };
        return (
          <div>
            <p style={{ marginBottom: '1rem', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
              Upload the <strong>GSTR-2A CSV</strong> from the GST portal. The app matches supplier GSTIN + invoice number against your Purchase Bills (₹2 tax tolerance).
            </p>
            <input ref={gstr2aInputRef} type="file" accept=".csv,text/csv" onChange={handleImport2A} style={{ display: 'none' }} />
            <button className="btn btn-primary" onClick={() => gstr2aInputRef.current?.click()} style={{ marginBottom: '1rem' }}>
              Import GSTR-2A CSV
            </button>
            {gstr2aRows.length > 0 && (
              <>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                  {[
                    { k: 'all', l: `All (${recon.length})` },
                    { k: 'matched', l: `Matched (${counts.matched})` },
                    { k: 'amount_mismatch', l: `Mismatch (${counts.amount_mismatch})` },
                    { k: 'portal_only', l: `Portal only (${counts.portal_only})` },
                    { k: 'book_only', l: `Book only (${counts.book_only})` },
                  ].map(s => (
                    <button key={s.k} type="button" className={gstr2aFilter === s.k ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'} onClick={() => setGstr2aFilter(s.k)}>{s.l}</button>
                  ))}
                </div>
                <div className="table-responsive">
                  <table className="data-table" style={{ width: '100%' }}>
                    <thead>
                      <tr>
                        <th>GSTIN</th><th>Supplier</th><th>Inv No</th><th>Date</th>
                        <th className="text-end">Portal Tax</th><th className="text-end">Book Tax</th><th>Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {visible.map((r, i) => (
                        <tr key={i}>
                          ...
                          <td>{r.invNo || '—'}</td>
                          <td>{r.date || '—'}</td>
                          <td className="text-end">{formatCurrency(r.tax || 0)}</td>
                          <td className="text-end">{r.bookTax ? formatCurrency(r.bookTax) : '—'}</td>
                          <td>
                            <span className={
                              r.status === 'matched' ? 'badge badge-success' :
                              r.status === 'amount_mismatch' ? 'badge badge-warning' :
                              r.status === 'portal_only' ? 'badge badge-secondary' : 'badge badge-danger'
                            }>{r.status.replace('_', ' ')}</span>
                          </td>
                        </tr>
                      ))}
                      {visible.length === 0 && (
                        <tr><td colSpan={7} style={{ textAlign: 'center', color: '#94a3b8' }}>No rows for this filter</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            {gstr2aRows.length === 0 && (
              <p style={{ color: '#64748b' }}>No GSTR-2A data imported yet.</p>
            )}
          </div>
        );
      })()}
      ...
      {activeTab === 'gstliability' && (() => {
        // Month-wise Output GST (from bills) vs Input (from purchases/expenses with GST)
        const byMonth = {};
        const add = (m, field, val) => {
          if (!m) return;
          if (!byMonth[m]) byMonth[m] = { output: 0, input: 0 };
          byMonth[m][field] += val;
        };
        (filteredBills || []).forEach(b => {
          ...
          const m = d.slice(0, 7);
          add(m, 'output', Number(b.totalTaxAmount) || 0);
        });
        (purchases || []).forEach(p => {
          const d = p.date || p.invoiceDate || '';
          const m = String(d).slice(0, 7);
          add(m, 'input', Number(p.taxAmount || p.tax || p.gstAmount || 0));
        });
        const months = Object.keys(byMonth).sort();
        let carry = 0;
        const rows = months.map(m => {
          const out = byMonth[m].output;
          const inp = byMonth[m].input;
          const net = out - inp;
          let liability = net - carry;
          let newCarry = 0;
          if (liability < 0) {
            newCarry = -liability;
            liability = 0;
          }
          const row = { m, out, inp, net, carryIn: carry, liability, carryOut: newCarry };
          carry = newCarry;
          return row;
        });
        return (
          <div>
            <p style={{ color: 'var(--text-muted)', marginBottom: '1rem' }}>
              Output GST (invoices) vs Input GST (purchases). Negative net creates ITC carry-forward to next month.
            </p>
            <table className="data-table" style={{ width: '100%' }}>
              <thead>
                <tr>
                  <th>Month</th>
                  <th className="text-end">Output GST</th>
                  <th className="text-end">Input GST (ITC)</th>
                  <th className="text-end">Net</th>
                  <th className="text-end">Carry-in</th>
                  <th className="text-end">Liability</th>
                  <th className="text-end">Carry-out</th>
                </tr>
              </thead>
              <tbody>
                {rows.map(r => (
                  <tr key={r.m}>
                    <td>{r.m}</td>
                    <td className="text-end">{formatCurrency(r.out)}</td>
                    <td className="text-end">{formatCurrency(r.inp)}</td>
                    <td className="text-end">{formatCurrency(r.net)}</td>
                    <td className="text-end">{formatCurrency(r.carryIn)}</td>
                    <td className="text-end" style={{ fontWeight: 700 }}>{formatCurrency(r.liability)}</td>
                    <td className="text-end">{formatCurrency(r.carryOut)}</td>
                  </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center' }}>No tax data in period</td></tr>}
              </tbody>
            </table>
          </div>
        );
