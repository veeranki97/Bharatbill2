...
export default function ClientsView({ onEdit, onDuplicate, onNew }) {
  ...
  const openEditClient = (client) => {
    // Guardian Protocol: lock name/GSTIN/state if active non-cancelled Tax Invoices exist
    const cBills = bills.filter(b =>
      (b.clientName || '').trim().toLowerCase() === (client.name || '').trim().toLowerCase()
      && b.status !== 'cancelled'
      && (b.invoiceType === 'tax-invoice' || b.invoiceType === 'Tax Invoice' || !b.invoiceType)
    );
    const locked = cBills.length > 0;
    setModalClient({ ...client, _locked: locked });
    setEditingClientId(client.id);
    setShowForm(true);
    if (locked) {
      // toast optional - avoid import issues if toast already used
    }
  };
  ...
  return (
    <div className="dashboard-container">
      ...
      <ClientModal show={showForm} onClose={closeForm} onSave={handleModalSave} client={modalClient} isEditing={!!editingClientId} defaultCountry={profileCountry} lockedFields={!!modalClient?._locked} />
